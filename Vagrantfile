...
Vagrant.configure("2") do |config|
  ...
  # VOVfile
  #
  # File with custom configuration that can be generated by VOV scripts - not to be modified manually, as it
  # could be overwritten. See Customfile description for details.
  if File.exists?(File.join(vagrant_dir,'VOVfile')) then
    eval(IO.read(File.join(vagrant_dir,'VOVfile')), binding)
  end
