...
tools_install() {
  ...
  if [[ "$(grunt --version)" ]]; then
    echo "Updating Grunt CLI"
		...
		npm update --verbose -g grunt-rtlcss 2>&1 | process_npm_verbose_output
  else
    echo "Installing Grunt CLI"
		...
		npm install --verbose -g grunt-rtlcss 2>&1 | process_npm_verbose_output
  fi
...
PHP
    ...
    echo "Running npm install for the first time, this may take several minutes..."
			noroot npm --verbose install 2>&1 | process_npm_verbose_output
  else
