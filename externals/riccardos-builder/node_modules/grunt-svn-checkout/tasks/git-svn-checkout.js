  ...
  grunt.registerMultiTask('svn_checkout', 'Checkout and update SVN repos with Grunt', function () {
    ...
    for ( var i = 0, length = repos.length; i < length; i++ ) {
      ...
      if ( ! grunt.file.isDir( relPath.outer ) ) {
        grunt.log.warn('The directory "' + relPath.outer + '" not found. Creating directories');
      }
      
      // Set up pull or clone
      if ( grunt.file.isDir( relPath.outer + path.sep  + '.svn' ) ) {
        cmd = [ 'update', relPath.outer ];
        grunt.log.ok( "Updating " + repos[ i ].repo +" to " + relPath.outer );
      } else {
        if ( ! grunt.file.isDir( relPath.outer ) ) {
          grunt.file.mkdir( relPath.outer );
          grunt.log.ok( "Creating directory " + relPath.outer );
        }
        cmd = [ 'checkout', repos[ i ].repo, relPath.outer ];
        grunt.log.ok( "Checking out " + repos[ i ].repo +" to " + relPath.outer );
      }
